...
# load dataset
# 3 stands for csv.QUOTE_NONE
# train_set = pd.read_csv("../../data/all_raw_train.csv", header = None, delimiter="\t", quoting=3, encoding='utf-8')

train_set = pd.read_csv("../../data/all_raw_train.csv", sep='\t')
val_set = pd.read_csv("../../data/all_raw_val.csv", sep='\t')


train_set = train_set[train_set['summary'].notna()]
train_set = train_set[train_set['source'].notna()]

val_set = val_set[val_set['summary'].notna()]
val_set = val_set[val_set['source'].notna()]
...
def compute_metrics(pred, tokenizer, rouge, bleu):
    ...
    print("--label_str--", type(label_str), np.array(label_str).shape)
    print("--pred_tokens--", type(pred_tokens), np.array(pred_tokens).shape)
    print("--refer_tokens--", type(refer_tokens), np.array(refer_tokens).shape)
